...
- `loadPage(direction, loadSize, syncStatus, filter)` — bidirectional keyset cursor pagination on `SyncableObjectService`, available on all platforms (KMP). `direction` is a `PageDirection` (`FromHead`, `Forward(cursor)`, or `Backward(cursor)`). Returns `PageResult<O>` with both `nextCursor` and `prevCursor`.
- `PageDirection` — sealed interface for `loadPage` direction: `FromHead`, `Forward(afterCursor)`, `Backward(beforeCursor)`. Sealed shape makes "backward from nothing" unrepresentable.
- `PageCursor` — opaque keyset cursor.
- `PageResult<O>` — one page of results: `items: List<O>`, `nextCursor: PageCursor?`, `prevCursor: PageCursor?`.
