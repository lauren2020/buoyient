...
- The `:syncable-objects` artifact (`com.les.buoyient:syncable-objects`) is the only required dependency. Add `:syncable-objects-hilt` for Hilt apps, `:testing` for tests and mock mode.
...
- Project-level configuration classes (`Buoyient`, `GlobalHeaderProvider`, `DatabaseProvider`, etc.) are in the `com.les.buoyient.globalconfigs` package.
