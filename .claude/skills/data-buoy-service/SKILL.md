...
- `ServerProcessingConfig.serviceHeaders` provides per-service HTTP headers. For auth headers shared across all services, use `DataBuoy.globalHeaderProvider` (a `GlobalHeaderProvider` lambda evaluated per-request).
