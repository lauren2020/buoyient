...
- Service-level configuration classes (`ServerProcessingConfig`, `SyncFetchConfig`, `SyncUpConfig`, `SyncUpResult`, `ConnectivityChecker`, `EncryptionProvider`, `PendingRequestQueueStrategy`, `SyncableObjectRebaseHandler`) are in the `com.les.databuoy.serviceconfigs` package.
- Data types used when interacting with `SyncableObjectService` (`HttpRequest`, `SyncableObjectServiceResponse`, `SyncableObjectServiceRequestState`, `GetResponse`, `CreateRequestBuilder`, `UpdateRequestBuilder`, `VoidRequestBuilder`, `ResponseUnpacker`, `SquashRequestMerger`) are in the `com.les.databuoy.syncableobjectservicedatatypes` package.
...
- `ServerProcessingConfig.serviceHeaders` provides per-service HTTP headers. For auth headers shared across all services, use `DataBuoy.globalHeaderProvider` (a `GlobalHeaderProvider` lambda evaluated per-request, in `com.les.databuoy.globalconfigs`).
