...
### 2. App restart
- **`SyncDriver.init{}`** calls `scheduleSyncIfNeeded()` to ensure WorkManager has a job enqueued in case the previous one completed/expired. However, on Android, WorkManager already handles this: `BuoyientInitializer` registers the service provider at app start, and if there are pending requests left from a prior session, `SyncScheduler` can be triggered. But actually — nothing currently triggers WorkManager on cold start *unless* `SyncDriver.init{}` does it. However, `SyncDriver.init{}` is called when each service is constructed (during app startup), so this is really just an "on service init, make sure background sync is scheduled" concern.
