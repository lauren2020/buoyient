 ...
 * synchronized map and a [Volatile] field.
 */
public class MockEndpointController {
    ...
    private val overrides = mutableMapOf<EndpointKey, FailureOverride>()
    ...
    public fun setOverride(serviceName: String, label: String, failure: FailureOverride) {
        synchronized(lock) { overrides[EndpointKey(serviceName, label)] = failure }
    }
    ...
    public fun clearOverride(serviceName: String, label: String) {
        synchronized(lock) { overrides.remove(EndpointKey(serviceName, label)) }
    }
    ...
    public fun activeOverride(serviceName: String, label: String): FailureOverride? =
        synchronized(lock) { overrides[EndpointKey(serviceName, label)] } ?: globalOverride
