...
import com.elvdev.buoyient.managers.PendingSyncRequest
...
public class SyncDriver<O : SyncableObject<O>, T : ServiceRequestTag> internal constructor(
        ...
        if (!connectivityChecker.isOnline()) {
            ...
            when (response) {
                ...
                is ServerManager.ServerManagerResponse.RequestTimedOut -> {
                    BuoyientLog.w(TAG, "Sync down failed due to connection error. Retrying later.")
                    return
                }

                is ServerManager.ServerManagerResponse.ServerError -> {
                    BuoyientLog.w(TAG, "Sync down failed due to server error (${response.statusCode}). Retrying later.")
                    return
                }

                is ServerManager.ServerManagerResponse.Failed -> {
                    BuoyientLog.w(TAG, "Sync down failed with status ${response.statusCode}. Retrying later.")
                    return
                }
                ...
                is ServerManager.ServerManagerResponse.Success -> {
                    ...
                    items.forEach { serverObj ->
                        try {
                            ...
                            BuoyientLog.e(TAG, "Failed to upsert item (${serverObj.serverId}): ", e)
                            skippedCount++
                        }
                    }
                    BuoyientLog.d(TAG, "Sync down complete: ${items.size} fetched, $upsertedCount upserted, $mergedCount merged, $conflictCount conflicts, $skippedCount skipped")
                }
            }
    }
    ...
    public suspend fun syncUpSinglePendingRequest(pendingRequestId: Int): Boolean {
        ...
        return try {
            ...
            BuoyientLog.e(TAG, "Error syncing $type for $clientId.", e)
            false
        }
    }
    ...
    private suspend fun syncUpPendingData(row: PendingSyncRequest<O>): Boolean {
        ...
        when (val resolution = request.resolveAllPlaceholders(
            ...
            is HttpRequest.PlaceholderResolutionResult.UnresolvedServerId -> {
                BuoyientLog.w(TAG, "Skipping ${row.type} for ${row.data.clientId}: serverId not yet resolved")
                return false
            }
            is HttpRequest.PlaceholderResolutionResult.UnresolvedCrossService -> {
                BuoyientLog.w(TAG, "Skipping ${row.type} for ${row.data.clientId}: cross-service dependency not yet resolved")
                return false
            }
        }
        return when (val response = serverManager.sendRequest(request)) {
            is ServerManager.ServerManagerResponse.ConnectionError,
            is ServerManager.ServerManagerResponse.RequestTimedOut -> {
                BuoyientLog.w(TAG, "Sync up failed due to connection error. Trying again later.")
            ...
            is ServerManager.ServerManagerResponse.ServerError -> {
                BuoyientLog.w(TAG, "Sync up failed due to server error (${response.statusCode}). Trying again later.")
                if (!row.serverAttemptMade) {
                    localStoreManager.pendingRequestQueueManager.markPendingRequestAsAttempted(row.pendingRequestId)
                }
                BuoyientLog.w(TAG, "Sync failed for pending_request_id: ${row.pendingRequestId} (${row.type}): ${response.statusCode} — it will be retried later.")
                false
            }

            is ServerManager.ServerManagerResponse.Failed -> {
                BuoyientLog.w(TAG, "Sync up received non-success status ${response.statusCode}.")
                if (!row.serverAttemptMade) {
                    localStoreManager.pendingRequestQueueManager.markPendingRequestAsAttempted(row.pendingRequestId)
                }
                BuoyientLog.w(TAG, "Sync failed for pending_request_id: ${row.pendingRequestId} (${row.type}): ${response.statusCode} — it will be retried later.")
                ...
                if (
                    ...
                    when (result) {
                        is SyncUpResult.Success -> {
                            ...
                            BuoyientLog.d(TAG, "Synced ${row.type} for ${row.data.clientId} (server_id=${result.data.serverId})")
                            true
                        }
                        is SyncUpResult.Failed.Retry -> {
                            ...
                            BuoyientLog.w(TAG, "Sync failed for ${row.type} for ${row.data.clientId} with pending_request_id: ${row.pendingRequestId} — it will be retried later.")
                            throw SyncUpRetryLaterException(
                        ...
                        is SyncUpResult.Failed.RemovePendingRequest -> {
                            ...
                            BuoyientLog.w(TAG, "Sync failed for ${row.type} for ${row.data.clientId} with pending_request_id: ${row.pendingRequestId}, pending request is being removed.")
                            false
                        }
                    }
                }
                ...
                syncDownJob = serviceScope.launch {
                    while (isActive) {
                        try {
                            ...
                            BuoyientLog.e(TAG, "Periodic sync-down failed: ", e)
                        }
                        delay(cadenceMs)
                    }
                }
