...
public class SyncUpCoordinator(
    ...
    public suspend fun syncUpAll(): Int {
        BuoyientLog.d(TAG, "Starting global sync up across ${drivers.size} services...")
        ...
        if (database.syncPendingEventsQueries.hasAnyConflicts().executeAsOne()) {
            BuoyientLog.w(TAG, "Skipping sync-up: unresolved conflicts exist. Resolve all conflicts before uploads can resume.")
            return 0
        }
        ...
        BuoyientLog.d(TAG, "Found ${globalQueue.size} pending sync rows globally.")
        ...
        for (entry in globalQueue) {
            ...
            if (driver == null) {
                BuoyientLog.w(TAG, "No service registered for '${entry.service_name}', skipping pending_request_id=${entry.pending_request_id}")
                continue
            }
            try {
        ...
        BuoyientLog.d(TAG, "Global sync complete: $syncedCount/${globalQueue.size} succeeded")
        return syncedCount
    }
