package com.les.buoyient.managers
...
import com.les.buoyient.utils.StorageCodec
...
internal class PendingRequestQueueManager<O : SyncableObject<O>, T : ServiceRequestTag>(
    ...
    private fun storeEntry(
        ...
        BuoyientLog.e(TAG, "storeEntry failed for service $serviceName", e)
    ...
    private fun replaceEntry(
        ...
        BuoyientLog.e(TAG, "replaceEntry failed for service $serviceName", e)
        QueueResult.StoreFailed
    }
    ...
    internal fun clearPendingRequestAfterUpload(
        ...
        BuoyientLog.e(TAG, "clearPendingRequestAfterUpload failed for service $serviceName (pendingRequestId=$pendingRequestId)", e)
        ...
        pendingRequests.forEach {
            ...
            when (rebaseMergeResult) {
                ...
                is SyncableObjectRebaseHandler.RebaseResult.Conflict -> {
                    BuoyientLog.w(TAG, "PendingSyncRequest (pending_request_id: ${it.pendingRequestId}) encountered a conflict on rebase, aborting rebase of subsequent pending requests until resolved.")
